      ...
      <div className="flex items-start gap-3 pr-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-400 mt-1 truncate">
            {isCompleted ? 'Task has been completed.' : 'Task is pending.'}
          </p>
          <div className="mt-2 flex flex-wrap gap-x-3 text-xs">
            <span>
              Priority: <span className="text-[#42ade2] font-medium">Normal</span>
            </span>
            <span>
              Status:{' '}
    ...
    <div className="flex gap-5 flex-1 min-h-0">
      ...
      <div className="bg-[#f5f8ff] rounded-2xl shadow-md flex flex-col w-[420px] shrink-0 overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-4 shrink-0">
          <h2 className="font-bold text-lg text-gray-800">My Tasks</h2>
          <div className="mt-1 w-8 h-0.5 bg-[#ff6767] rounded-full" />
        </div>

        {/* List */}
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">
          {todos.length === 0 ? (
            <p className="text-center text-gray-400 text-base py-8">No tasks yet.</p>
          ) : (
            todos.map((todo) => (
              <TaskListCard
                key={todo._id}
